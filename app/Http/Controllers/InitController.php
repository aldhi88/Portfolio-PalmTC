...
// index =====================================================
    ...
    public function dt()
    {
        ...
        ->withCount([
            // 'tc_init_bottles as total_block' => function(Builder $q){
            //     $q->where('status',1)
            //         ->select(DB::raw('COUNT(DISTINCT(block_number))'));
            // },
            // 'tc_init_bottles as total_bottle' => function(Builder $q){
            //     $q->where('status',1);
            // },
        ])
        ->with([
        ...
        return DataTables::eloquent($data)
            ...
            //     return $data->tc_init_bottles[0]->tc_medium_stocks->tc_mediums->code;
            // })
            // ->addColumn('medium_stock_date', function($data){
            //     $q = TcInit::getMediumStock($data->id);
            //     $return = null;
            //     foreach ($q as $key => $value) {
            //         $return .= $key!=0?'<br>':null;
            //         $return .= $value->tc_medium_stocks->created_at_short_format;
            //     }
            //     return $return;
            // })
