...
<script>
    ...
    function initDt(obsDate = null){
        var dtTable = $('#myTable').DataTable({
            processing: true,serverSide: true,pageLength: 50,order: [ [0,'desc'] ],destroy: true,
            columnDefs: [
                { className: 'text-center', targets: ['_all'] },
            ],
            ajax: {
                url:'{{ route("embryo-obs.dtCreate") }}',
                data:{
                    obsId:'{{ $data["obsId"] }}',
                    initId:'{{ $data["initId"] }}',
                    obsDate:obsDate,
            ...
            columns: [
                ...
                { data: 'form_embryo', name: 'form_embryo', orderable:false, searchable:false},
    ...
    function initFormObs(){
        $('body').on('change','input.form-obs',function(){
            ...
            formData.tc_embryo_ob_id = '{{ $data["obsId"] }}';
            formData.tc_embryo_bottle_id = $(this).attr('data-id');
            formData.sub = $(this).attr('data-sub');
            ...
            $.ajax({
                ...
                url: '{{ route("embryo-obs.storeObDetail") }}',
                data: formData,
                success: function(a) {
                    $('#alert-area2').empty();
