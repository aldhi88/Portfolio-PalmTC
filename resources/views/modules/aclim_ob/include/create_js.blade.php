...
<script>
    $('#startObs').submit(function (e) {
        ...
        formData.append('tc_aclim_id', '{{ $data["tc_aclim_id"] }}'); 
        formData.append('tc_init_id', '{{ $data["initId"] }}'); 
        $.ajax({
            type: 'POST',
            url: "{{ route('aclim-obs.store') }}",
            data: formData,
    ...
    function initDt(){
        var dtTable = $('#myTable').DataTable({
            ...
            ajax: {
                url: '{{ route("aclim-obs.dtObs") }}',
                data: function(d){ // note !
                    d.aclimId = '{{ $data["tc_aclim_id"] }}';
                    d.obsId = '{{ $data["obsId"] }}';
                }
            },
            columns: [
    ...
    function initFormObs(){
        $("body").on('click','input.item', function() {
            ...
            $.ajax({ //note!
                ...
                url: '{{ route("aclim-obs.storeDetail") }}',
                data: formData,
                success: function(a) {
    ...
    $("body").on('click','input.checkbox', function() {
        ...
        formData.aclimId = '{{ $data["tc_aclim_id"] }}';
        formData.obsId = '{{ $data["obsId"] }}';
        formData.initId = '{{ $data["initId"] }}';
        ...
        $.ajax({ //note!
            type: 'POST', cache: false, processData: true,
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            url: '{{ route("aclim-obs.storeDetailAll") }}',
            data: formData,
            success: function(a) {
                $('#myTable').DataTable().ajax.reload(function(){ loader(false)},false);
                loader(false);
