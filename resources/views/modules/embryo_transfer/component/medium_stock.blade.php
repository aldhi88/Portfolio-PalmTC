...
<div class="col">
    <table id="myTable" class="table table-striped table-bordered nowrap table-xs w-100">
        ...
        <tbody>
            ...
            @if (in_array($item['tc_bottle_id'],$data['allowBottle']))
                @php
                    $total = $item['current_stock'];
                    $usedCallus = 0;
                    $usedSolid = 0;
                    $usedSuspen = 0;
                    foreach ($data['medStockCallus'] as $key => $value) {
                        if($item['id'] == $value['id']){
                            $usedCallus = $value['used_stock'];
                        }
                    }
                    foreach ($data['medStockSolid'] as $key => $value) {
                        if($item['id'] == $value['id']){
                            $usedSolid = $value['used_stock'];
                        }
                    }
                    foreach ($data['medStockSuspen'] as $key => $value) {
                        if($item['id'] == $value['id']){
                            $usedSuspen = $value['used_stock'];
                        }
                    }
                    $bottleLeft = $total-$usedCallus-$usedSolid-$usedSuspen;
                @endphp
                <tr>
