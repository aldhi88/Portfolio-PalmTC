...
function genStep1(page=null){
    $.ajax({
        ...
        url: '{{ route("embryo-transfers.getStep1") }}',
        data: { page:page,initId:'{{ $data["initId"] }}' },
        success: function(a) {
            $('#step1').html(a);
        },
        error: (a) => {
            alert("Error #003, EmbryoTransfer-getStep1 function is invalid.");
        }
    });
}
genStep2();
function genStep2(page=null){
    $.ajax({
        type: 'GET', cache: false, contentType: false, processData: true,
        url: '{{ route("embryo-transfers.getStep2") }}',
        data: { page:page,initId:'{{ $data["initId"] }}' },
        success: function(a) {
            $('#step2').html(a);
        },
...
function genStep3(page=null){
    $.ajax({
        type: 'GET', cache: false, contentType: false, processData: true,
        url: '{{ route("embryo-transfers.getStep3") }}',
        data: { page:page,initId:'{{ $data["initId"] }}' },
        success: function(a) {
            $('#step3').html(a);
        },
        error: (a) => {
            alert("Error #003, EmbryoTransfer-getStep3 function is invalid.");
        }
    });
}
genStep4();
function genStep4(page=null){
    $.ajax({
        type: 'GET', cache: false, contentType: false, processData: true,
        url: '{{ route("embryo-transfers.getStep4") }}',
        data: { page:page,initId:'{{ $data["initId"] }}' },
        success: function(a) {
            $('#step4').html(a);
        },
        error: (a) => {
            alert("Error #003, EmbryoTransfer-getStep4 function is invalid.");
        }
    });
}
...
$('#finishTransfer').submit(function (e) {
    ...
    $.ajax({
        type: 'POST',
        url: "{{ route('embryo-transfers.finishTransfer') }}",
        data: formData,
