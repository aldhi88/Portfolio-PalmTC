...
<script>
    var dtTable = $('#myTable').DataTable({
        ...
        ajax: {
            url: "{{ route('matur-transfers.dtShow') }}",
            data: {initId:'{{ $data["initId"] }}'}
        },
        columns: [
            { data: 'bottle_date_format', name: 'tc_matur_bottles.bottle_date', orderable:true, searchable:false},
            { data: 'work_date_format', name: 'tc_matur_obs.work_date', orderable:true, searchable:false},
            { data: 'tc_matur_bottles.alpha', name: 'tc_matur_bottles.alpha', orderable:false, searchable:false},
            { data: 'bottle_matur', name: 'bottle_matur', orderable:false, searchable:false},
    ...
    var dtTable2 = $('#myTable2').DataTable({
        ...
        ajax: {
            url: "{{ route('matur-transfers.dtShow2') }}",
            data: {initId:'{{ $data["initId"] }}'}
        },
        columns: [
            ...
            { data: 'to_germin', name: 'to_germin', orderable:false, searchable:false},
...
$("#myTable2").on("click","button.printByTransfer", function(){
    ...
    window.open('{{ route("matur-transfers.printByTransfer") }}?id='+transferId);
})
</script>
